...
import { Input, Flex, Image, Box as Boxx, Progress} from '@chakra-ui/react';
...
export class EnergyAccountDetails extends React.Component<Props, State> {
    ...
    fetchIntervalsNight(startTimeStamp?: string, endTimeStamp?: string) {
        ...
        for (let i = 1; i < 11; i++){
        const sixPmUnixNight = (Math.floor(new Date().setHours(17,0,0,0))/1000 - 24*60*60*(i+1))
        const sixAmUnixNight = (Math.floor(new Date().setHours(6,0,0,0))/1000 - 24*60*60*i)
        ...
        + '/intervals?' 
        + new URLSearchParams({
            ...(sixPmUnixNight ? {'start_date': '' + sixPmUnixNight} : null),
            ...(sixAmUnixNight ? {'end_date': '' + sixAmUnixNight} : null)
        })
    ...
    calculateNight() {  
        ...
        if (!this.state.usageListNight || this.state.usageListNight.length != 10) return;
        ...
        if (131 - this.state.milestowork! * 0.5 - average_usage_night  > 0){  // this.consumption is the utility used  
            ...
            this.setState({canRun: "With your current commute distance and electricity usage for this property the Ford F-150 Lightning can successfully power this property overnight and cover your commute the next day!"})
            
        } else {
            this.setState({canRunHome: false}) 
            this.setState({canRun: "Unfortunately, with your current commute distance and electricity usage for this property the Ford F-150 Lightning can’t cover your commute after powering this property overnight."})
    ...
    calculate() {  
        ...
        if (!this.state.usageList || this.state.usageList.length != 10) return;


        const sum_of_usage = this.state.usageList.reduce((a, b) => a + b, 0)
